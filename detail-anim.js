// ORDO LAYER 상세페이지 애니메이션
// 카페24 스마트디자인 /js/detail-anim.js
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (stepVideo) stepVideo.playbackRate = 1.3;
    ...
    (function(){
        ...
        function checkFade() {
              if(triggered) return;
              var rect = s4overlay.getBoundingClientRect();
              if(rect.top < window.innerHeight * 0.85 && rect.bottom > 0) {
                      triggered = true;
                      fadeItems.forEach(function(item, i) {
                                setTimeout(function() { item.classList.add('s4-visible'); }, i * 320);
});
                      return;
              }
    ...
    (function(){
      var cards = document.querySelectorAll('.s1-card');
        ...
        function blinkCardText(card) {
              var content = card.querySelector('.s1-card-content');
              if(!content) return;
              var count = 0; var total = 4;
              var interval = setInterval(function() {
                      content.style.opacity = (count % 2 === 0) ? '0.1' : '1';
                      count++;
                      if(count >= total) { clearInterval(interval); content.style.opacity = '1'; }
              }, 120);
        }
        cards.forEach(function(card) {
              var done = false;
              function check() {
                      ...
                      if(rect.top < window.innerHeight * 0.85 && rect.bottom > 0) {
                                done = true; blinkCardText(card); return;
                      }
